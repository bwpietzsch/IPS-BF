...
globals[
  ...
  ncut                   ; # of trees infested that were removed
  ...
  nlocked1               ; # of beetles locked by detention devices in generation 1
  ...
  nlocked2               ; # of beetles locked by detention devices in generation 2
  ...
  nlocked3               ; # of beetles locked by detention devices in generation 3
  ...
  data8                  ; loaded GIS data - calculated tree timber volume
...
patches-own [
  ...
  volume             ; timber volume [m3] calculated beforehand using height and dbh
]
...
to setup
  ...
  ifelse seed = 0 [ ; check if a fixed seed is to be used
  ; let seed be completely random
  ][
    random-seed seed ; set seed to given value
  ]
  ...
  ifelse seed = 0 [
  ][ ; do nothing if seed is 0
    random-seed new-seed ; set seed to random to get different dispersal and infestation patterns for each run
  ]

  ; remove faulty source trees: all trees that are more than 125 m away from the target infestation
  ; pattern located in the middle of the simulation world
  ask patch 50 50 [
    ask patches with [distance myself > 25 and inf != 0 and inf != 5] [
  ...
  ; remove faulte source trees from agent list source
  ask source with [pcolor = black] [
    set source (source with [self != myself])
  ]

  ; only during model calibration
;  set swarming (7 + random 50)
;  set meanenergy (precision (6 + random-float 8) 1)
;  set generations (1 + random 2)
;  set winddirection (precision (random-float 360) 1)
;  set windspeed (precision (random-float 5.5) 1)
...
to import-GIS-data
  ...
  carefully [
    set data1 (gis:load-dataset word path-to-input "spruceprop.asc") ; import spruce proportions from stand data (asc)
  ][
    error "No spruce proportion data found! Use 'poly.asc' as file name for import from entered file path."
  ]
  gis:apply-raster data1 spruceprop ; assign information on the spruce proportion of each given stand to enveloped patches
  ...
  carefully [
    set data8 gis:load-dataset word path-to-input "timber-volume.asc" ; import tree timber volume
  ][
    error "No tree timber volume data found! Use 'timber-volume.asc' as file name for import from entered file path."
  ]
  gis:apply-raster data8 volume
...
to create-environment
  ...
  ask patches [
    ifelse (primattract <= 0) or (primattract >= 0) [ ; check for NaNs in data (patches outside given research area)
      ...
      if spruceprop < random-float 1 and inf != 3 [ ; create randomly distributed spruce trees in accordance with forest inventory data
  ...
  ask patches [
    ...
    if inf = 5 [
      set pcolor (pcolor + 40) ; change coloring of trees outside nlp
    ]
  ]
...
to cut-infested-trees
  let sfn round ((count host) * sanitation-felling / 100) ; calculate number of cut trees
  ask n-of sfn host [
...
to calculate-results
  if ngeneration = 1 [
    ...
    set maxpercept ( perceptdist + diffusionrate ) ; calculate beetles max perception distance
  ]
  if ngeneration = 2 [
    set beetle_percentsucc2 ((beetle_infesting / beetle_emerged) * 100) ; calculate proportion of successful beetles

    set nlocked2 (beetle_locked - nlocked1)
  ...
  if ngeneration = 3 [
    ...
    set nlocked3 (beetle_locked - nlocked1 - nlocked2)
...
to export-infestation-pattern
  ...
  gis:store-dataset export-raster (word path-to-output behaviorspace-run-number ".asc")
...
470
285
\"0\" for competely random environment based on provided GIS data; \"any number\" for identical environment for each run
12
0.0
1
...
@#$#@#$#@
<experiments>
  <experiment name="calib-blackforest1" repetitions="200" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>meanenergy</metric>
    <metric>generations</metric>
    <metric>swarming</metric>
    <metric>winddirection</metric>
    <metric>windspeed</metric>
    <metric>nlocked1</metric>
    <metric>nlocked2</metric>
    <metric>nlocked3</metric>
    <metric>nhost</metric>
    <metric>nright</metric>
    <metric>nshould</metric>
  ...
  <experiment name="calib-blackforest2" repetitions="200" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>meanenergy</metric>
    <metric>generations</metric>
